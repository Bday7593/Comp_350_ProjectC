...
_interrupt:
	push bp
	mov bp, sp
	mov ax, [bp+4]	;gets the interrupt number in AL
...
intr:	int #0x00	;call the interrupt (00 will be changed above)
	...
	mov ah, #0		;we only want the AL returned
	pop bp
	ret

;void makeinterrupt21()
;this sets up the interrupt 0x21 vector
;when an interrupt 0x21 is called in the furutre,
...
;this is called to stasrt a program that is loased into memory.
;void launchProgram(int segment)
_launchProgram:
	mov bp, sp
	mov bx,[bp+2]; get the segment into bx

	mov ax,cs	;modify the jmp to ur segment
	mov ds,ax 	;this is self-modeifying code
	mov si,#jump
